// game1 剧情分支处理
// 对应原 src/views/games/game1/composables/useStoryHandler.js
// 依赖 js/game1/store.js、js/game1/story.js、js/game1/assets.js（先于本文件加载）
...
function handleChoice(val) {
  ...
  if (val === "L1_A") {
    ...
    store.setGameTimeout(() => {
      store.pushMessage({ sender: "二大爷", text: "😡", extraClass: "bad-msg-1" });
    }, 3000);
    store.setGameTimeout(() => {
      store.pushMessage({ sender: "老爸", text: "😡", extraClass: "bad-msg-1" });
    }, 4000);
    store.setGameTimeout(() => {
      store.pushMessage({ sender: "大堂哥", text: "😡", extraClass: "bad-msg-1" });
    }, 5000);
    store.setGameTimeout(() => {
      store.pushMessage({
        type: "sys",
      ...
      store.state.groupName = "相亲相爱一家人 (26)";
    }, 7000);
    store.setGameTimeout(() => {
    ...
    store.pushMessage({
      sender: "我",
      image: ASSETS.IMAGES.signal_monster,
      type: "image",
      isMe: true,
    });
    store.setGameTimeout(() => {
  ...
  // ----------- 第三关 -----------
    ...
    store.setGameTimeout(() => {
      store.pushMessage({ type: "sys", text: "3 分钟后...", extraClass: "bad-msg-3" });
    }, 5000);
    store.setGameTimeout(() => {
      store.pushMessage({
        sender: "老爸",
    ...
    store.setGameTimeout(() => {
      store.pushMessage({ sender: "二大爷", text: "以后遇到要求屏幕共享的，要警惕！" });
    }, 12500);
    store.setGameTimeout(() => {
      store.pushMessage({ sender: "老妈", text: "简直防不胜防！" });
    }, 14500);
    store.setGameTimeout(() => {
      store.pushMessage({ sender: "老爸", text: "幸好没上当😌多亏了咱家孩子！" });
    }, 16500);
    store.setGameTimeout(() => {
      store.pushMessage({
        sender: "老妈",
