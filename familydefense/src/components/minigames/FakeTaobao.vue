<template>
  <div class="taobao-overlay" v-show="store.state.activeOverlay === 'taobao'">
    <div class="taobao-header">
      ...
      <div class="fake-url" :class="{ 'taobao-flaw-revealed': flaws.url }" @click="reveal('url', '✅ 找到破绽：正规官方网站绝不会包含 free、xyz 等奇怪后缀！')">
    ...
    <div class="taobao-content">
      ...
      <div class="countdown-box" :class="{ 'taobao-flaw-revealed': flaws.countdown }" @click="reveal('countdown', '✅ 找到破绽：利用虚假倒计时制造焦虑，逼迫受害者失去理智。')">
      ...
      <div class="fake-form">
        <div style="font-weight: bold; margin-bottom: 10px; color: #333">邮寄信息 (仅需支付 19.9 元邮费)</div>
        <input type="text" class="form-input" placeholder="收件人姓名" disabled />

        <div class="danger-input-wrapper" :class="{ 'taobao-flaw-revealed': flaws.password }" @click="reveal('password', '✅ 找到破绽：正规平台绝不会在网页直接索要银行卡密码！')">
          <input type="password" class="form-input danger-input" placeholder="请输入银行卡取款密码以验证身份" disabled />
        </div>
...
const reveal = (key, hint) => {
  ...
  const foundCount = ['url', 'countdown', 'password'].filter(k => flaws.value[k] === true).length;
